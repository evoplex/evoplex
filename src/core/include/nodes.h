...
public:
    ...
    static Nodes fromCmd(const QString& cmd, const AttributesScope& attrsScope,
            const int graphType, QString* errMsg = nullptr, std::function<void(int)> progress = [](int){});
...
private:
    ...
    static QStringList validateHeader(const QString& header, const AttributesScope& attrsScope, QString* errMsg = nullptr);

    static NodePtr readRow(const int row, const QStringList& header, const QStringList& values,
                           const AttributesScope& attrsScope, const bool isDirected, QString* errMsg = nullptr);
};
